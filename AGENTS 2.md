...
Branching approach (practical)
- Keep `main` production‑stable. Do all active work on `development`.
...
- Mark phase docs as draft/WIP while on `development`. If the docs site is public, gate WIP sections until merged to `main`.
- Tag releases from `main` (e.g., v1.1.0) so MVP consumers can pin stable versions.

Docs publishing
- The public documentation site is built from the Jekyll docs branch.
- Author and iterate docs on `development`; promote to the Jekyll docs branch when stable.
